...
/* 
Validação do módulo de Logs de Professores:
...
router.get('/statsP', (req, res) => {
  Logs.find({ roles: { $all: ['professor'] } }).then(logs => {
    const UmaSemana = new Date().getTime() - 7 * 24 * 60 * 60 * 1000
    const numLogsProfessores = logs.length
